  ...
  return (
    <div className="flex h-screen bg-map-paper overflow-hidden font-military">
      <div className="flex-1 relative flex flex-col">
          ...
          {takeGroundOption && (
            <div className={`absolute left-1/2 -translate-x-1/2 z-50 text-center uppercase transition-all duration-300 ${dismissedOverlay ? 'top-2' : 'top-1/2 -translate-y-1/2'}`}>
              <div className={`bg-white border-4 border-blue-600 rounded-xl shadow-2xl ${dismissedOverlay ? 'p-3 flex items-center gap-4' : 'p-10'}`}>
                ...
                {!dismissedOverlay && takeGroundOption?.overrun && <p className="text-xs mt-1 font-bold normal-case text-red-600">Průlom: po obsazení může tank ještě jednou zaútočit.</p>}
              ...
              <div className="bg-white/95 p-4 border-2 border-map-ink-blue shadow-lg rounded">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  {gameState.phase === 'attack' && (
                    <>
                      <button disabled={!gameState.units[selected].overrunReady && (gameState.units[selected].resources === 0 || gameState.units[selected].hasAttacked)} onClick={() => setActType('attack')} className={`w-full p-2 rounded border text-[10px] font-bold ${actType === 'attack' ? 'bg-red-200' : 'bg-white disabled:opacity-50 uppercase'}`}>{gameState.units[selected].overrunReady ? 'PRŮLOM' : 'ÚTOK'}</button>
        ...
        {/* Ovládání tahu počítače: výběr rychlosti + ruční spuštění. Panel je
            nahoře ve volném prostoru, ať nezakrývá tácky zdrojů dole. */}
        {isAiTurn && !gameState.winner && (
          <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-white/95 border-2 border-red-800 rounded-xl shadow-2xl px-4 py-2.5">
            <div className="flex items-center gap-1.5">
              <Bot size={18} className="text-red-800" />
            ...
            <div className="flex items-center gap-1" title="Rychlost tahu počítače">
              {AI_SPEEDS.map(s => (
                <button key={s.id} onClick={() => pickAiSpeed(s.id)}
                  className={`px-2 py-1 rounded text-[10px] font-bold uppercase transition-colors ${aiSpeedId === s.id ? 'bg-slate-800 text-white' : 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-100'}`}>
                  {s.label}
                </button>
              ))}
            </div>
            {aiRun ? (
              <span className="text-[11px] text-slate-600 italic">Počítač táhne…</span>
            ) : (
              <button onClick={() => setAiRun(true)}
                className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-red-800 text-white font-bold uppercase text-[11px] hover:bg-red-700 transition-colors shadow-md">
        ...
        {/* Resource UI area – viditelná i během tahu AI (hráč vidí, kam
            počítač rozděluje; tlačítka jsou pro něj stejně zamčená). */}
        ...
        <div className="fixed bottom-4 left-4 z-50 flex flex-col gap-2 pointer-events-none">
           <div className="flex gap-2 pointer-events-auto">
              ...
              {!!aiPlayerId && aiLog.length > 0 && (
                <div className="flex items-center gap-1 px-2 bg-white border-2 border-red-800/40 rounded-lg shadow-lg" title="Prohlídka tahu počítače – hrací deska se vrátí do stavu před krokem, barevně se označí, co s jednotkou AI provedla. Stav hry se nemění.">
                  <Bot size={14} className="text-red-800" />
                  <button onClick={() => setAiReviewIdx(0)}
                    disabled={aiReviewIdx === 0} title="Na začátek tahu"
                    className="px-1 py-1.5 rounded text-slate-700 hover:bg-slate-100 disabled:opacity-30 transition-colors">
                  ...
                  <span className="text-[10px] font-black text-slate-800 whitespace-nowrap min-w-[30px] text-center">
                  ...
                  {aiReviewEntry ? (
                    <>
                      <span className="text-[10px] text-slate-600 max-w-[160px] truncate">{aiReviewEntry.label}</span>
                      <button onClick={() => setAiReviewIdx(null)} title="Zpět na živý pohled" className="px-1 py-1.5 rounded text-slate-700 hover:bg-slate-100 transition-colors">
                        <CloseIcon size={12} />
                      </button>
                    </>
                  ) : (
                    <span className="text-[9px] uppercase font-bold text-slate-500 whitespace-nowrap pr-1">akcí PC</span>
                  )}
                </div>
              )}
